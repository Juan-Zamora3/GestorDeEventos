// src/modulos/administradorEventos/componentes/IncioEvento/GridEventosAdminEventos.tsx
...
const GridEventosAdminEventos: React.FC<Props> = ({
  eventos,
  stagger = false,
  onEventoClick,
}) => {
  ...
  return (
    <section className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 h-full">
      {eventos.map((evento, idx) => (
        <article
          key={evento.id}
          className={`bg-white rounded-3xl overflow-hidden cursor-pointer relative transform-gpu transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1 hover:shadow-lg ${
            stagger
              ? entered
          ...
          style={{
            aspectRatio: "4 / 3",
            ...(stagger ? { transitionDelay: `${idx * 120}ms` } : {}),
          }}
          onClick={() =>
            onEventoClick
              ? onEventoClick(evento.id)
              : navigate(`/admin-eventos/evento/${evento.id}`)
          }
        >
