...
const ModalPreguntaFormulario: FC<Props> = ({
  abierto,
  modo,
  pregunta,
  onGuardar,
  ...
  const [obligatorio, setObligatorio] = useState<boolean>(
    pregunta?.obligatorio ?? false,
  );
  const [opcionNueva, setOpcionNueva] = useState<string>("");
  const [opciones, setOpciones] = useState<string[]>(
    pregunta?.config?.opciones ?? [],
  );

  // 🔹 Sincroniza el formulario cuando cambias de pregunta a editar
  useEffect(() => {
    if (!abierto) return;
    setNombre(pregunta?.nombre ?? "");
    setTipo(pregunta?.tipo ?? "texto_corto");
    setPlaceholder(pregunta?.placeholder ?? "");
    setObligatorio(pregunta?.obligatorio ?? false);
    setOpciones(pregunta?.config?.opciones ?? []);
    setOpcionNueva("");
  }, [pregunta, abierto]);
  ...
  const showOpciones =
    tipo === "seleccion_simple" || tipo === "seleccion_multiple";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!nombreTrim) {
      alert("La pregunta debe tener un texto.");
      return;
    }

    if (showOpciones && opciones.length === 0) {
      alert("Debes agregar al menos una opción para este tipo de campo.");
      return;
    }

    const config =
      showOpciones && opciones.length > 0 ? { opciones } : undefined;
  ...
  const addOpcion = () => {
    ...
    if (opciones.some((o) => o.toLowerCase() === val.toLowerCase())) {
  ...
  const handleChangeTipo = (nuevo: TipoCampo) => {
    setTipo(nuevo);
    // Si ya no es tipo selección, limpiamos opciones
    if (nuevo !== "seleccion_simple" && nuevo !== "seleccion_multiple") {
      setOpciones([]);
      setOpcionNueva("");
    }
  };
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 p-4 sm:p-6 overflow-y-auto">
      <div className="relative bg-white rounded-3xl shadow-xl w-full max-w-[680px] max-h-[80vh] overflow-y-auto py-6 px-8">
        {modo === "editar" && pregunta?.id && (
          <button
            type="button"
            onClick={() => onEliminar && onEliminar(pregunta.id)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                value={tipo}
                onChange={(e) =>
                  handleChangeTipo(e.target.value as TipoCampo)
                }
              >
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`h-5 w-10 rounded-full transition ${
                obligatorio ? "bg-[#5B4AE5]" : "bg-slate-300"
              }`}
            >
              <span
                className={`block h-5 w-5 bg-white rounded-full shadow transform transition ${
                  obligatorio ? "translate-x-5" : "translate-x-0"
                }`}
              />
            </button>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={addOpcion}
                  className="px-3 py-2 rounded-lg bg-gradient-to-r from-[#5B4AE5] to-[#7B5CFF] text-white text-xs font-semibold"
                >
                  Añadir
                </button>
              </div>
