// src/modulos/administradorEventos/componentes/ModalCampoEvento.tsx
...
const ModalCampoEvento: FC<Props> = ({
  abierto,
  modo,
  campo,
  onGuardar,
  ...
  const [tipo, setTipo] = useState<CampoEvento["tipo"]>(
    campo?.tipo ?? "opciones",
  );

  const noEditable = !!campo?.immutable;

  // ✅ Sincronizar cuando cambias de campo o abres/cerras el modal
  useEffect(() => {
    if (!abierto) return;
    setNombre(campo?.nombre ?? "");
    setTipo(campo?.tipo ?? "opciones");
  }, [campo, abierto]);

  if (!abierto) return null;

  const titulo =
    modo === "crear" ? "Agregar nueva variable" : "Editar variable";
  const textoAccion = modo === "crear" ? "Agregar" : "Guardar";

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const base: CampoEvento = {
      id: campo?.id ?? "",
      nombre: nombre.trim(),
      tipo,
      immutable: campo?.immutable,
    };
    onGuardar(base);
  };
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 p-4 sm:p-6 overflow-y-auto">
      <div className="relative bg-white rounded-3xl shadow-xl w-full max-w-[640px] max-h-[80vh] overflow-y-auto py-6 px-8">
        {modo === "editar" && campo?.id && !noEditable && (
          <button
            type="button"
            onClick={() => onEliminar && onEliminar(campo.id)}
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <select
              ...
              value={tipo}
              onChange={(e) =>
                setTipo(e.target.value as CampoEvento["tipo"])
              }
              disabled={noEditable}
            >
