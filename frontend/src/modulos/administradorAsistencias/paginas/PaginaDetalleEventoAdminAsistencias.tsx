...
const PaginaDetalleEventoAdminAsistencias: React.FC = () => {
  ...
  // 👇 que coincida con la ruta: /admin-asistencias/eventos/:idEvento
  const { idEvento } = useParams<{ idEvento: string }>();

  // Por ahora título fijo, luego lo puedes leer desde InfoEventoPanel / Firestore
  const tituloEvento =
    idEvento === "concurso-robotica"
        ...
        <header className="flex-shrink-0">
          ...
          >
            ...
            <nav className="mt-6 px-6 sm:px-10 pb-0">
              <div
                ref={navRef}
                className="relative w-full bg-[#E5E9F6] rounded-2xl px-6 py-3"
              >
                ...
                <span
                  className={`absolute bottom-0 left-0 h-2 rounded-full bg-gradient-to-r from-[#5B5AE5] to-[#7B5CFF] transition-[transform,width] duration-500 ease-[cubic-bezier(0.22,1.0,0.28,1)] ${
                    glow
                      ? "drop-shadow-[0_0_12px_rgba(91,74,229,0.7)]"
                      : ""
                  }`}
                  style={{
...
// Export default para que App.tsx lo pueda importar así:
// import PaginaDetalleEventoAdminAsistencias from "...";
